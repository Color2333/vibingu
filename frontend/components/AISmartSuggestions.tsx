    ...
    return (
      <div className={`glass-card p-6 ${className}`}>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-semibold text-[var(--text-primary)]">AI 智能建议</h3>
        </div>
        <div className="text-center py-8">
          <div className="text-4xl mb-3">💡</div>
          <p className="text-[var(--text-secondary)] mb-4">点击生成个性化 AI 建议</p>
          <button
    ...
    return (
      <div className={`glass-card p-6 ${className}`}>
        ...
        <div className="text-center py-8">
          ...
          <p className="text-[var(--text-secondary)]">正在生成建议...</p>
          <p className="text-xs text-[var(--text-tertiary)] mt-1">AI 正在分析你的数据</p>
        </div>
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className={`glass-card p-6 ${className}`}>
        ...
        <p className="text-[var(--text-secondary)] text-center py-4">暂无建议</p>
      </div>
    );
  }
    ...
    <div className={`glass-card p-6 ${className}`}>
      ...
      {data.focus_area && (
        <div className="mb-4 p-4 rounded-xl bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-500/20">
          ...
          <p className="text-[var(--text-primary)] font-medium">{data.focus_area}</p>
          {data.focus_reason && (
            <p className="text-xs text-[var(--text-secondary)] mt-1">{data.focus_reason}</p>
          )}
        </div>
      )}
      ...
      <div className="space-y-3">
        {data.suggestions.map((suggestion, idx) => (
          <div
            key={idx}
            className="p-4 rounded-xl bg-[var(--glass-bg)] border border-[var(--border)] hover:bg-[var(--bg-secondary)] transition-colors"
          >
